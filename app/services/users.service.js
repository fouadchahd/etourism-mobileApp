...
export const getUsersCount=async()=>{
    return  getUsers({itemsPerPage:1,page:1}).then(({data}) => data["hydra:totalItems"]);
}
export const getTouristCount=async()=>{
    return  getTourists({itemsPerPage:1,page:1}).then(({data}) => data["hydra:totalItems"]);
}
export const getAdminCount=async()=>{
    return  getAdmins({itemsPerPage:1,page:1}).then(({data}) => data["hydra:totalItems"]);
}
